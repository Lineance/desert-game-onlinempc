...
def test_markov_predictor_shape_and_value_range(level3_cfg):
    ...
    scenarios = predictor.sample_scenarios(current_weather=Weather.SUNNY, horizon=5, n_scenarios=4)
...
def test_markov_predictor_reproducible(level4_cfg):
    ...
    assert p1.sample_scenarios(Weather.HOT, horizon=6, n_scenarios=3) == p2.sample_scenarios(
...
def test_transition_matrix_must_be_3x3():
    ...
    try:
        ...
        raise AssertionError("Expected ValueError for non-3x3 transition matrix")
